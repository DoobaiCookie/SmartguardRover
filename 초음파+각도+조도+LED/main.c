...
if (is_avoiding == 0 && is_emergency == 0)
...
is_avoiding = 0; // 2. "나 이제 회피 안 해"라고 선언

break; // 3. 루프 탈출 (함수 밖으로 나감)

}

if (HAL_GetTick() - last_avoid_light_check > 500)

{

Check_Light(); // 여기서도 불 켜고 끌 수 있게 호출!

last_avoid_light_check = HAL_GetTick();
...
is_emergency = 0; // 비상 상황 해제

is_avoiding = 0; // 회피 모드도 같이 초기화

last_cmd_time = HAL_GetTick(); // 타임아웃 리셋

printf("System Recovered! Back to Normal.\r\n");

continue; // 즉시 정상 루프로 복귀
...
// 아직도 뒤집혀 있다면 경고 메시지 출력하고 대기

printf("!! EMERGENCY !! Robot Tipped Over...\r\n");

HAL_Delay(200);

continue; // 아래 코드 실행 안 하고 여기서 뻉뺑이

}

if (HAL_GetTick() - last_light_check_time > 500)

{

Check_Light();

last_light_check_time = HAL_GetTick();
