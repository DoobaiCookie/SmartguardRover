...
// 센서 1 (TIM1 사용) 변수
long unsigned int echo_time1;
int dist1;

// 센서 2 (TIM2 사용) 변수
long unsigned int echo_time2;
int dist2;
/* USER CODE END PD */
...
long unsigned int echo1(void)
{
   ...
   __HAL_TIM_SET_COUNTER(&htim1, 0); // TIM1 카운터 초기화

   while(HAL_GPIO_ReadPin(ECHO1_GPIO_Port, ECHO1_Pin) == HIGH);

   echo = __HAL_TIM_GET_COUNTER(&htim1); // TIM1 값 읽기
...
void trig2(void)
{
   ...
   delay_us(10);
   HAL_GPIO_WritePin(TRIG2_GPIO_Port, TRIG2_Pin, LOW);
}
...
long unsigned int echo2(void)
{
   ...
   // TIM2 사용
   while(HAL_GPIO_ReadPin(ECHO2_GPIO_Port, ECHO2_Pin) == LOW){}

   __HAL_TIM_SET_COUNTER(&htim2, 0); // TIM2 카운터 초기화

   while(HAL_GPIO_ReadPin(ECHO2_GPIO_Port, ECHO2_Pin) == HIGH);

   echo = __HAL_TIM_GET_COUNTER(&htim2); // TIM2 값 읽기
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    timer_start();
    printf("Dual Ultrasonic Sensor Start\n");
    /* USER CODE END 2 */
    ...
    /* Infinite loop */
      ...
      while (1)
      {
          ...
          // 결과 출력
          printf("S1: %d mm, S2: %d mm\n", dist1, dist2);

          // 다음 루프 전 딜레이 (100ms 정도)
          HAL_Delay(100);
