...
static void MX_GPIO_Init(void) { GPIO_InitTypeDef g = {0}; __HAL_RCC_GPIOC_CLK_ENABLE(); __HAL_RCC_GPIOD_CLK_ENABLE(); __HAL_RCC_GPIOA_CLK_ENABLE(); __HAL_RCC_GPIOB_CLK_ENABLE(); HAL_GPIO_WritePin(GPIOC, LED_LEFT_Pin|LED_RIGHT_Pin|TRIG1_Pin, 0); HAL_GPIO_WritePin(GPIOA, TRIG2_Pin|LBF_Pin|LFB_Pin, 0); HAL_GPIO_WritePin(GPIOB, LBB_Pin|TRIG3_Pin|TRIG4_Pin|LFF_Pin|RBF_Pin|RBB_Pin|RFF_Pin|RFB_Pin, 0); g.Pin = B1_Pin; g.Mode = GPIO_MODE_IT_RISING; g.Pull = GPIO_NOPULL; HAL_GPIO_Init(B1_GPIO_Port, &g);
...
// 나머지 핀 설정
g.Pin = ECHO2_Pin; g.Mode = GPIO_MODE_INPUT; HAL_GPIO_Init(ECHO2_GPIO_Port, &g); g.Pin = TRIG2_Pin|LBF_Pin|LFB_Pin; g.Mode = GPIO_MODE_OUTPUT_PP; g.Speed = GPIO_SPEED_FREQ_LOW; HAL_GPIO_Init(GPIOA, &g); g.Pin = ECHO1_Pin|ECHO3_Pin|ECHO4_Pin; g.Mode = GPIO_MODE_INPUT; HAL_GPIO_Init(GPIOB, &g); g.Pin = LBB_Pin|TRIG3_Pin|TRIG4_Pin|LFF_Pin|RBF_Pin|RBB_Pin|RFF_Pin|RFB_Pin; g.Mode = GPIO_MODE_OUTPUT_PP; HAL_GPIO_Init(GPIOB, &g); g.Pin = LED_LEFT_Pin|LED_RIGHT_Pin|TRIG1_Pin; HAL_GPIO_Init(GPIOC, &g); HAL_NVIC_SetPriority(EXTI1_IRQn, 0, 0); HAL_NVIC_EnableIRQ(EXTI1_IRQn); HAL_NVIC_SetPriority(EXTI9_5_IRQn, 0, 0); HAL_NVIC_EnableIRQ(EXTI9_5_IRQn); HAL_NVIC_SetPriority(EXTI15_10_IRQn, 0, 0); HAL_NVIC_EnableIRQ(EXTI15_10_IRQn); }
